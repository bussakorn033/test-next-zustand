		...
		switch (variant) {
			...
			case 'ghost-icon-negative-no-padding':
				return css`
					...
					box-sizing: border-box;
					background: var(--button-${variant.replace('-icon', '').replace('-no-padding', '')});
					color: var(--button-color-${variant.replace('-icon', '').replace('-no-padding', '')});
					&:hover {
						background: var(--button-hover-${variant.replace('-icon', '').replace('-no-padding', '')});
					}
					&:active {
						background: var(--button-active-${variant.replace('-icon', '').replace('-no-padding', '')});
					}
					&:disabled *,
					&:disabled {
						background: ${variant.includes('-icon')
							? 'var(--button-color-disabled-ghost-icon)'
							: `var(--button-disabled-${variant.replace('-icon', '').replace('-no-padding', '')})`};
						outline: none;
						&,
						* {
							color: var(--button-color-disabled-${variant.replace('-icon', '')}) !important;
						}
					}
				`;
			default: // primary
